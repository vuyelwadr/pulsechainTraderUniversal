#!/usr/bin/env python3
"""
Strategy 101: Rainbow Charts Binary UpWave

Pine note references Rainbow Charts Oscillator. We derive a binary up/down
wave from the Rainbow Oscillator (ROSC): up when ROSC>0, down when ROSC<0.
...
class Strategy101RainbowChartsBinaryUpWave(BaseStrategy):
    def __init__(self, parameters: Dict=None):
        ...
        super().__init__('Strategy_101_RainbowChartsBinaryUpWave', params)
    ...
    def calculate_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        denom = (c.rolling(10, min_periods=1).max() - c.rolling(10, min_periods=1).min()).replace(0, np.nan)
        avg10 = (sma2 + dsma2 + tsma2 + qsma2 + psma2 + ssma2 + s2sma2 + osma2 + o2sma2 + desma2) / 10.0
        data['rosc'] = 100.0 * (c - avg10) / denom
        data['rosc'] = data['rosc'].replace([np.inf,-np.inf], np.nan).fillna(0)
        data['up_wave'] = data['rosc'] > 0
        return data
