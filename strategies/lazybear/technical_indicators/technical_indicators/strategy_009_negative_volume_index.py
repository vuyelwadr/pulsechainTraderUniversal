#!/usr/bin/env python3
"""
Strategy 009: Negative Volume Index (NVI)
...
class Strategy009NegativeVolumeIndex(BaseStrategy):
    def __init__(self, parameters: Dict=None):
        params={'ma_period':255,'signal_threshold':0.6}
        if parameters: params.update(parameters)
        super().__init__('Strategy_009_NVI', params)
    def calculate_indicators(self, data:pd.DataFrame)->pd.DataFrame:
        ...
        for i in range(1,len(data)):
            if data['volume'].iloc[i] < data['volume'].iloc[i-1]:
                ...
                nvi.iloc[i]=nvi.iloc[i-1]*(1+change)
            else:
                nvi.iloc[i]=nvi.iloc[i-1]
        data['nvi']=nvi
        data['nvi_ma']=data['nvi'].rolling(self.parameters['ma_period'],min_periods=1).mean()
        return data
    def generate_signals(self, data:pd.DataFrame)->pd.DataFrame:
        p=self.parameters
        data['buy_signal']=crossover(data['nvi'], data['nvi_ma'])
        data['sell_signal']=crossunder(data['nvi'], data['nvi_ma'])
        data['signal_strength']=calculate_signal_strength([((data['nvi']-data['nvi_ma']).abs()/data['nvi_ma'].replace(0,np.nan)).fillna(0).clip(0,1)],[1.0])
