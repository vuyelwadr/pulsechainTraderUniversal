#!/usr/bin/env python3
"""
Strategy 081: RSI using Last Open

Compute RSI on the open price series instead of close.
...
class Strategy081RSIUsingLastOpen(BaseStrategy):
    def __init__(self, parameters: Dict=None):
        params={'period':14,'overbought':70,'oversold':30,'signal_threshold':0.6}
        if parameters: params.update(parameters)
        super().__init__('Strategy_081_RSI_Using_Last_Open', params)
    def calculate_indicators(self, data:pd.DataFrame)->pd.DataFrame:
        if 'open' not in data.columns and 'price' in data.columns: data['open']=data['price']
        n=int(self.parameters['period'])
        delta=data['open'].diff(); gain=delta.clip(lower=0); loss=-delta.clip(upper=0)
        avg_g=pine_rma(gain,n); avg_l=pine_rma(loss,n)
        rs=avg_g/avg_l.replace(0,np.nan); data['rsi_open']=(100-100/(1+rs)).fillna(50)
        return data
    def generate_signals(self, data:pd.DataFrame)->pd.DataFrame:
        ob=float(self.parameters['overbought']); os_=float(self.parameters['oversold'])
        buy=crossover(data['rsi_open'], os_); sell=crossunder(data['rsi_open'], ob)
        data['buy_signal']=buy; data['sell_signal']=sell
        st=((data['rsi_open']-50).abs()/50.0).clip(0,1).where(buy|sell,0.0)
