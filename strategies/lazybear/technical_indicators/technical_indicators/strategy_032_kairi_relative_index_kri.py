#!/usr/bin/env python3
"""
Strategy 032: Kairi Relative Index (KRI)

TradingView URL: https://www.tradingview.com/v/xzRPAboO/
Type: momentum

Description:
KRI = (Close - SMA(n)) / SMA(n) * 100. Signals on zero cross with trend filter.
"""
...
class Strategy032KRI(BaseStrategy):
    def __init__(self, parameters: Dict=None):
        params={'period':14,'signal_threshold':0.6,'trend_ma':50}
        if parameters: params.update(parameters)
        super().__init__('Strategy_032_KRI', params)

    def calculate_indicators(self, data:pd.DataFrame)->pd.DataFrame:
        if 'close' not in data.columns and 'price' in data.columns:
            data['close']=data['price']
        sma=data['close'].rolling(self.parameters['period'],min_periods=1).mean()
        data['kri']=(data['close']-sma)/sma.replace(0,np.nan)*100.0
    ...
    def generate_signals(self, data:pd.DataFrame)->pd.DataFrame:
        ...
        up=crossover(data['kri'],0) & (data['close']>data['trend_sma'])
        dn=crossunder(data['kri'],0) & (data['close']<data['trend_sma'])
        data['buy_signal']=up; data['sell_signal']=dn
        mag=(data['kri'].abs()/data['kri'].abs().rolling(50,min_periods=1).max().replace(0,np.nan)).fillna(0).clip(0,1)
