#!/usr/bin/env python3
"""
Strategy 172: Hurst Cycle Channel Clone Oscillator

Pine reference: pine_scripts/172_cycle_channel_osc.pine

Signals: buy when oshort crosses above 0.5 (midline), sell when crosses below 0.5.
"""
...
class Strategy172CycleChannelOscillator(BaseStrategy):
    def __init__(self, parameters: Dict=None):
        ...
        super().__init__('Strategy_172_CycleChannelOscillator', params)
    ...
    def calculate_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        scmm = (sct + scb) / 2.0
        data['omed'] = (scmm - mcb) / (mct - mcb).replace(0,np.nan)
        data['oshort'] = (data['close'] - scb) / (sct - scb).replace(0,np.nan)
        data[['omed','oshort']] = data[['omed','oshort']].clip(0,1).fillna(0)
        return data

    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        buy = crossover(data['oshort'], 0.5)
        sell = crossunder(data['oshort'], 0.5)
        data['buy_signal'] = buy
        data['sell_signal'] = sell
        st = (data['oshort'] - 0.5).abs() * 2.0
