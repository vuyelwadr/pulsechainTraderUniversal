"""Balanced version of SegmentTrendHold optimized for all market conditions."""
...
class SegmentTrendHoldBalancedV1(BaseStrategy):
    """
    Balanced trend holding strategy that adapts to different market conditions.
    Moderate selectivity with good risk management for 1.5% fee environment.
    """

    requires_regime_data: bool = True

    def __init__(self, parameters: Dict | None = None) -> None:
        # Balanced parameters - middle ground approach
        defaults = {
            "timeframe_minutes": 240,  
            "trade_amount_pct": 0.7,   # Balanced position size
            "entry_strength_threshold": 0.10,  # Balanced selectivity
            "min_confirm_strength": 0.18,      # Reasonable confirmation
            "exit_strength_threshold": -0.06,  # Balanced exit timing
            "exit_delay_bars": 8,              # Moderate exit speed
            "require_confirm": True,          # Require confirmation but reasonable
            "confirm_timeframe_minutes": 240,
            "confirm_exit": True,             # Confirm exits
            "allow_early_entry": False,        # Conservative early entries
            "confirm_grace_bars": 10,         # Reasonable grace period
            "max_drawdown_pct": 0.25,          # Balanced risk management
            "trail_atr_mult": 1.8,            # Balanced trailing stop
            "time_stop_bars": 500,             # Reasonable max hold period
            "reentry_cooldown_bars": 6,        # Balanced cooldown
        }
        if parameters:
            defaults.update(parameters)
        super().__init__("SegmentTrendHoldBalancedV1", defaults)
    ...
    def parameter_space(cls) -> Dict[str, Tuple[float, float]]:
        return {
            "entry_strength_threshold": (0.06, 0.18),
            "min_confirm_strength": (0.12, 0.28),
            "exit_strength_threshold": (-0.12, -0.02),
            "exit_delay_bars": (4, 12),
            "max_drawdown_pct": (0.15, 0.35),
            "trail_atr_mult": (1.2, 2.8),
            "time_stop_bars": (300, 700),
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Extract parameters
        exit_delay = max(1, int(self.parameters.get("exit_delay_bars", 8)))
        require_confirm = bool(self.parameters.get("require_confirm", True))
        confirm_exit = bool(self.parameters.get("confirm_exit", True))
        entry_strength_threshold = float(self.parameters.get("entry_strength_threshold", 0.10))
        exit_strength_threshold = float(self.parameters.get("exit_strength_threshold", -0.06))
        allow_early_entry = bool(self.parameters.get("allow_early_entry", False))
        confirm_grace_bars = int(self.parameters.get("confirm_grace_bars", 10))
        min_confirm_strength = float(self.parameters.get("min_confirm_strength", 0.18))
        max_drawdown_pct = float(self.parameters.get("max_drawdown_pct", 0.25))
        trail_atr_mult = float(self.parameters.get("trail_atr_mult", 1.8))
        time_stop_bars = int(self.parameters.get("time_stop_bars", 500))
        reentry_cooldown_bars = int(self.parameters.get("reentry_cooldown_bars", 6))
        ...
        for i in range(len(df)):
            ...
            if not holding:
                ...
                # Entry during ongoing strong but confirmed trend
                if not can_buy and state == "UPTREND" and reentry_cooldown_remaining == 0:
                    if (confirm_ok and strength is not None and 
                        strength >= (entry_strength_threshold * 1.3)):
                        can_buy = True
                        signal_strength = min(0.6, strength / (entry_strength_threshold * 1.3))
