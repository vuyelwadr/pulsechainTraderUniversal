...
class BacktestEngine:
    ...
    def _calculate_results(self, data: pd.DataFrame) -> Dict:
        ...
        if self.portfolio_history:
            ...
            underwater = portfolio_df['total_value'] < running_max
            current_start = None
            max_duration_bars = 0
        ...
        if strategy is not None:
            ...
            if buy_samples:
                results['avg_buy_step_pct'] = float(np.mean(buy_samples) * 100.0)
                results['buy_step_sample_count'] = len(buy_samples)
            if sell_samples:
                results['avg_sell_step_pct'] = float(np.mean(sell_samples) * 100.0)
                results['sell_step_sample_count'] = len(sell_samples)
